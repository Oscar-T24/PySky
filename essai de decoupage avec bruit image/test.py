...
lowess_values = lowess(median_y_values, x, frac=0.1, is_sorted=True)
lowess_x, lowess_y = lowess_values.T

# Draw the lowess line on the final image
lowess_coords = list(zip(lowess_x, lowess_y))
draw.line(lowess_coords, fill=(0, 255, 0), width=2)
...
for i in range(pix.shape[1]):
    ...
    # Get lowess regression line for current column
    lowess_y_values = lowess(col.ravel(), x, frac=0.03, return_sorted=False)
    
    # Filter pixels below lowess line
    if len(lowess_y_values) > 0:
        col_y_values = spl(x[i])
        for j in range(len(col_y_values)):
            if col_y_values[j] < lowess_y_values[j]:
                lower_half_mask[j:, i] = 0
                break
...
# Save the final image with the interpolated line and lowess line
