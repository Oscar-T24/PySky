...
with open('departements-france.csv','r') as f:
    read = csv.DictReader(f,delimiter=',',fieldnames=['code_departement','nom_departement','code_region','nom_region'])
    for ligne in read:
        if ligne != 'nom_departement':
            coordonees.append({'Code':ligne['code_departement'],'departement':ligne['nom_departement'],'coordonnee':list(departement_coordinate(ligne['nom_departement']))}) #.encode('utf-8')
