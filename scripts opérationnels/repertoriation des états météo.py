...
with open('temperature_data.csv', 'r') as file1, open('etats_departements.csv', 'r') as file2:
    reader1 = csv.reader(file1)
    reader2 = csv.reader(file2)
    next(reader1) # skip header row in file1
    next(reader2) # skip header row in file2
    data1 = [row for row in reader1]
    data2 = [row for row in reader2]

# Convert the CSV data into Pandas DataFrames
df1 = pd.DataFrame(data1, columns=['Code', 'Temperature'])
df2 = pd.DataFrame(data2, columns=['Code', 'etat'])

# Convert the columns to the appropriate data types
df1['Code'] = str(df1['Code'])
df1['Temperature'] = str(df1['Temperature'])
df2['Code'] = str(df2['Code'])

# Replace missing values in the data with NaN values
df1 = df1.replace('NA', float('nan'))
df2 = df2.replace('NA', float('nan'))

# Merge the DataFrames on the common descriptor column
